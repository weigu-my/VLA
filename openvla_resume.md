...
### 推理速度与动作平滑度（Pi0 vs Pi0.5，RTX 3090）

| 指标 | Pi0 | Pi0.5 |
| --- | ---: | ---: |
| 单次 chunk 推理延迟 | 121 ms | 140 ms |
| 有效动作吞吐（一次出 50 步）| 412 actions/s | 358 actions/s |
| 峰值显存 | 8.4 GB | 17.0 GB |
| 动作 L1 变化率 mean\|Δa\| | 0.0337 | **0.0280**（↓17%）|
| 动作 jerk mean\|Δ²a\| | 0.0397 | **0.0266**（↓33%）|

> 推理速度：Pi0/Pi0.5 单次推理（VLM 前缀 + 10 步 flow matching 去噪）一次输出 50 步动作 chunk，有效动作吞吐约 360–410 actions/s，相比 OpenVLA 单步自回归（每次 1 个动作、约 5–10 actions/s）高出约 40–80 倍，体现 action chunking 的吞吐优势。动作平滑度：在相同 task/seed/初始状态、两模型均成功的 10 个 episode（步数可比 99 vs 95）上测得，Pi0.5 的 L1 变化率与 jerk 均显著低于 Pi0，控制更平顺，与其在精细接触抓取任务上的优势一致。

### 去噪步数消融（Pi0.5，LIBERO-Spatial，replan=5，官方论文未公开此消融）

| 去噪步数 | 成功率(50-ep) | 单次推理延迟 |
| --- | ---: | ---: |
| 1 | 98.0% | 104.7 ms |
| 2 | 96.0% | 106.6 ms |
| 5 | 100.0% | 115.1 ms |
...
> 结论：成功率对去噪步数不敏感（1–50 步均在 94–100%，差异落在 50-ep 采样噪声内、无单调趋势），而延迟随步数近似线性增长（固定开销约 102ms 为 VLM 前缀一次前向 + KV 缓存，每多一步去噪仅 +2.9ms，因其只过 300M Action Expert）。即在该任务上去噪步数可从默认 10 步降至 1–5 步、近乎不损成功率而降低延迟，印证了 flow matching VLA 的单步化（如 SnapFlow 蒸馏）可行性。注：50-ep 为小样本，成功率差异无统计显著性。
